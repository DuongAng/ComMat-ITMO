...
public class Logarithm_function {
    ...
    public void init(double[][] function_Table) {
        ...
        for (int i = 0; i < function_Table.length; i++) {
            ...
            sum_x_y += Math.log(function_Table[i][0]) * function_Table[i][1];
        }
    ...
    public void solve() {
        ...
        Function function = x -> (ress[0] * Math.log(x) + ress[1]);

        result = new Result();
        result.setname_Function("Logarithmic");
        result.setstr_Function(ress[0] + "ln(x) + " + ress[1]);
