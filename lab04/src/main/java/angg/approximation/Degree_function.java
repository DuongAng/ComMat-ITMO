...
public class Degree_function {
    ...
    public void init(double[][] function_Table) {
        ...
        for (int i = 0; i < function_Table.length; i++) {
            sum_x += Math.log(function_Table[i][0]);
            sum_x2 += Math.pow(Math.log(function_Table[i][0]), 2);
            sum_y += Math.log(function_Table[i][1]);
            sum_x_y += Math.log(function_Table[i][0]) * Math.log(function_Table[i][1]);
        }
    ...
    public void solve() {
        ...
        Function function = x -> (Math.exp(ress[1]) * Math.pow(x, ress[0]));

        result = new Result();
        result.setname_Function("Power");
        result.setstr_Function(Math.exp(ress[1]) + "x^" + ress[0]);
